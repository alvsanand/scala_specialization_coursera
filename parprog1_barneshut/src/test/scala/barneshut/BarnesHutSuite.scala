...
import FloatOps._
  ...
  test("Simulator.mergeBoundaries") {
    val boundariesA = new Boundaries()
    boundariesA.minX = 0
    boundariesA.minY = 0
    boundariesA.maxX = 100
    boundariesA.maxY = 100

    val boundariesB = new Boundaries()
    boundariesB.minX = 50
    boundariesB.minY = 50
    boundariesB.maxX = 150
    boundariesB.maxY = 150

    val sim = new Simulator(null, null)

    val boundariesAB = sim.mergeBoundaries(boundariesA, boundariesB)
    assert(boundariesAB.minX === 0, s"bad minX")
    assert(boundariesAB.minY === 0, s"bad minY")
    assert(boundariesAB.maxX === 150, s"bad maxX")
    assert(boundariesAB.maxY === 150, s"bad maxY ===")
  }
