...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val (massX, massY) = ((bodies.map(b => b.x * b.mass).reduce(_ + _) / mass): Float,
                          (bodies.map(b => b.y * b.mass).reduce(_ + _) / mass): Float)
    val total: Int = bodies.size

    def insert(b: Body): Quad = if (size > minimumSize) {
      val quadSize = size / 4
      var fork = Fork(Empty(centerX - quadSize, centerY - quadSize, quadSize * 2),
        Empty(centerX + quadSize, centerY - quadSize, quadSize * 2),
        Empty(centerX - quadSize, centerY + quadSize, quadSize * 2),
        Empty(centerX + quadSize, centerY + quadSize, quadSize * 2))

      (bodies :+ b).foreach(q => fork = fork.insert(q))
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        case Empty(_, _, _) =>
        // no force
        case Leaf(_, _, _, bodies) => bodies.filter(_ != this).foreach(x => addForce(x.mass, x.x, x.y))
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val bX = if (b.x < boundaries.minX) 0
      else if (b.x > boundaries.maxX) (sectorPrecision - 1)
      else ((b.x - boundaries.minX) / sectorSize).toInt
      val bY = if (b.y < boundaries.minY) 0
      else if (b.y > boundaries.maxY) (sectorPrecision - 1)
      else ((b.y - boundaries.minY) / sectorSize).toInt

      apply(if (bX >= sectorPrecision) (sectorPrecision - 1) else bX, if (bY >= sectorPrecision) (sectorPrecision - 1) else bY) += b

      this
    }

    def apply(x: Int, y: Int) = {
      if ((y * sectorPrecision + x) > matrix.length)
        println(s"SectorMatrix: ${y * sectorPrecision} + $x -> ${matrix.length} vs ${y * sectorPrecision + x}")
