...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    newBoundaries.minX = boundaries.minX
    newBoundaries.maxX = boundaries.maxX
    newBoundaries.minY = boundaries.minY
    newBoundaries.maxY = boundaries.maxY

    if (body.x > boundaries.minX) newBoundaries.minX = body.x
    if (body.x < boundaries.maxX) newBoundaries.maxX = body.x

    if (body.y > boundaries.minY) newBoundaries.minY = body.y
    if (body.y < boundaries.maxY) newBoundaries.maxY = body.y
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(Seq[Body]())((a, b) => (a :+ b.updated(quad)) , (a, b) => a ++ b)
  }
